<template>
    <div class="blog-category-container" v-loading="loading">
        ...
        <RightList v-if="data.length > 0" :list="list" @select="handleSelect"></RightList>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        list() {
            ...
            const result = [{ name: '全部分类', id: -1, artcleCount: totalArtcleCount }, ...this.data]
            // console.log(this.data);
            const res = result.map(it => {
                return { ...it, isSelected: it.id === this.categroyId, categroyid: this.categroyId }
            })
            return res
        }
    },
