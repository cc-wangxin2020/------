  ...
  <div class="blog-list-container" v-loading="loading" ref="blogListContainer">
    <ul>
      <li v-for="item in data.row" :key="item.id">
        ...
        <div class="content">
          <div class="title" @click="handleClickToDetails(item)">
            {{ item.title }}
          </div>
          <div class="detail">
            ...
            <span class="category" @click="handleClickCate(item)">{{
              item.category.name
            }}</span>
          </div>
          <div class="description" @click="handleClickToDetails(item)">
            {{ item.descriptions }}
          </div>
        </div>
      </li>
...
export default {
  ...
  mounted() {
    this.$refs.blogListContainer.addEventListener("scroll", this.getScrollTop);
    this.$EventBus.$on("BackTop", this.scorllTop);
  },
  destroyed() {
    this.$refs.blogListContainer.addEventListener("scroll", this.getScrollTop);
    this.$EventBus.$off("BackTop", this.scorllTop);
  },
  computed: {
    routeInfo() {
  ...
  methods: {
    formatData,
    getScrollTop() {
      this.$EventBus.$emit("scrollTop", this.$refs.blogListContainer);
    },
    async getData() {
    ...
    scorllTop() {
      ...
      console.log(this.scorllTopValue);
    },
  ...
  watch: {
    $route: {
      async handler() {
        (this.loading = true), (this.data = await this.getData());
        this.loading = false;
        console.log("路由变化");
      },
      deep: false,
